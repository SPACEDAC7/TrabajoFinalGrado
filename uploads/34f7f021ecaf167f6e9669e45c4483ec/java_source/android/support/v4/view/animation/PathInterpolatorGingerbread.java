...
implements Interpolator {
    ...
    public PathInterpolatorGingerbread(float f2, float f3) {
        this(PathInterpolatorGingerbread.createQuad(f2, f3));
    }

    public PathInterpolatorGingerbread(float f2, float f3, float f4, float f5) {
        this(PathInterpolatorGingerbread.createCubic(f2, f3, f4, f5));
    }
    ...
    public float getInterpolation(float f2) {
        ...
        float f3 = this.mX[n3] - this.mX[n2];
        if (f3 == 0.0f) {
            return this.mY[n2];
        }
        f2 = (f2 - this.mX[n2]) / f3;
        f3 = this.mY[n2];
        return (this.mY[n3] - f3) * f2 + f3;
    }
}
