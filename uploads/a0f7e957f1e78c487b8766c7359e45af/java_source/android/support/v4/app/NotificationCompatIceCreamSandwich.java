...
class NotificationCompatIceCreamSandwich {
     ...
     */
    static Notification add(Context context, Notification notification, CharSequence charSequence, CharSequence charSequence2, CharSequence charSequence3, RemoteViews remoteViews, int n2, PendingIntent pendingIntent, PendingIntent pendingIntent2, Bitmap bitmap, int n3, int n4, boolean bl) {
        ...
        if ((notification.flags & 128) != 0) {
            bl2 = true;
            return context.setFullScreenIntent(pendingIntent2, bl2).setLargeIcon(bitmap).setNumber(n2).setProgress(n3, n4, bl).getNotification();
        }
        bl2 = false;
        return context.setFullScreenIntent(pendingIntent2, bl2).setLargeIcon(bitmap).setNumber(n2).setProgress(n3, n4, bl).getNotification();
    }
