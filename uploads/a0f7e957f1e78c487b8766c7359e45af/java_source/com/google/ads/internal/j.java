...
public class j {
    ...
    public static ViewGroup a(Activity activity, d d2) {
        try {
            GestureStore gestureStore = new GestureStore();
            gestureStore.load((InputStream)new ByteArrayInputStream(a));
            GestureOverlayView gestureOverlayView = new GestureOverlayView((Context)activity);
            gestureOverlayView.setGestureStrokeType(0);
            gestureOverlayView.setEventsInterceptionEnabled(false);
            gestureOverlayView.setGestureVisible(false);
            gestureOverlayView.addOnGesturePerformedListener((GestureOverlayView.OnGesturePerformedListener)new a(activity, d2, gestureStore));
            return gestureOverlayView;
        }
        catch (IOException var0_1) {
            b.a("Gestures disabled: Error loading gesture data.", var0_1);
            return null;
        }
    }
    ...
    implements GestureOverlayView.OnGesturePerformedListener {
        ...
        public a(Activity activity, d d2, GestureStore gestureStore) {
            this.b = activity;
            this.c = d2;
            this.a = gestureStore;
        }
         ...
         */
        public void onGesturePerformed(GestureOverlayView object, Gesture iterator) {
            object = this.a.recognize(iterator);
            iterator = object.iterator();
            while (iterator.hasNext()) {
                Prediction prediction = (Prediction)iterator.next();
            ...
            if (object.size() == 0) {
                ...
                if (((Prediction)object.get((int)0)).score <= 2.0 || !"debug".equals(((Prediction)object.get((int)0)).name) || this.c == null) return;
                {
                    object = this.c.c() == null ? "[No diagnostics available]" : this.c.c();
                }
            }
            new AlertDialog.Builder((Context)this.b).setMessage((CharSequence)object).setTitle((CharSequence)"Ad Information").setPositiveButton((CharSequence)"Share", new DialogInterface.OnClickListener((String)object){
                final /* synthetic */ String a;

                public void onClick(DialogInterface dialogInterface, int n2) {
                    a.this.b.startActivity(Intent.createChooser((Intent)new Intent("android.intent.action.SEND").setType("text/plain").putExtra("android.intent.extra.TEXT", this.a), (CharSequence)"Share via"));
                }
